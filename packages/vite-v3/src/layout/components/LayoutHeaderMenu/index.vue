...
<!--    <ul class="menu">
    ...
    <template v-for="menu in asyncRoutes">
      <ul v-if="menu.menu_id === $route.meta.parentId[0]" :key="menu.menu_id" class="menu">
      ...
      <template v-for="list in menu.children">
        <ul v-if="list.menu_id === $route.meta.parentId[1]" :key="list.menu_id" class="submenu">
...
.layout-header-menu {
  overflow: hidden;
  margin: $layout-spacing-y $layout-spacing-x 0;
  ...
  > .menu {
    display: flex;
    height: $layout-header-menu-height;
    line-height: $layout-header-menu-height;
